...
class BuyStockView: UIView {
    ...
    private var amountStocksLabel = UILabel()
    var amountStocksTextField = UITextField()
    ...
    private var labelNoteTitle = UILabel()
    ...
    weak var viewController: JournalViewController? {
        didSet {
            ...
            NotificationCenter.default.publisher(for: UITextField.textDidChangeNotification, object: amountStocksTextField)
                .map { $0.object as? UITextField }
                .compactMap { $0?.text }
                .sink { [weak self] stocksAmount in
                    self?.viewController?.numberOfStocks = Int(stocksAmount) ?? 0
                }.store(in: &allCancellables)
            ...
            NotificationCenter.default.publisher(for: UITextField.textDidChangeNotification, object: noteTextView)
                .map { $0.object as? UITextField }
                .compactMap { $0?.text }
                .sink { [weak self] note in
                    self?.viewController?.noteText = note
                }.store(in: &allCancellables)
        }
    }
...
private extension BuyStockView {
    func loadUI() {
        setuptTickerStockSection()
        setupAmountStocks()
        setuplabelDate()
        setupPriceStockLabel()
        setupNoteSection()
    ...
    //MARK: setupAmountStocks
    func setupAmountStocks() {
        self.amountStocksLabel = CustomLabel()
        self.amountStocksLabel.text = "Количество"
        self.amountStocksLabel.textColor = .secondaryLabel
        self.addSubview(self.amountStocksLabel)
        
        NSLayoutConstraint.activate([
            self.amountStocksLabel.leftAnchor.constraint(
                equalTo: self.leftAnchor,
                constant: 20),
            self.amountStocksLabel.rightAnchor.constraint(
                equalTo: self.rightAnchor,
                constant: -20),
            self.amountStocksLabel.topAnchor.constraint(equalTo: tickerStockTextField.bottomAnchor , constant: 8)
        ])
        self.amountStocksTextField = CustomTextField()
        self.amountStocksTextField.delegate = self
        self.addSubview(self.amountStocksTextField)
        NSLayoutConstraint.activate([
            amountStocksTextField.leftAnchor.constraint(
                equalTo: self.leftAnchor,
                constant: 20),
            amountStocksTextField.rightAnchor.constraint(
                equalTo: self.rightAnchor,
                constant: -20),
            amountStocksTextField.heightAnchor.constraint(
                equalToConstant: 45),
            amountStocksTextField.topAnchor.constraint(
                equalTo: amountStocksLabel.bottomAnchor ,constant: 8)
        ])
    ...
    private func setuplabelDate() {
        ...
        NSLayoutConstraint.activate([
            ...
            labelDate.topAnchor.constraint(
                equalTo: amountStocksTextField.bottomAnchor ,constant: 8)
        ])
    }
    
    //MARK: setupPriceStockLabel
    private func setupPriceStockLabel() {
    ...
    //MARK: setupNoteSection
    private func setupNoteSection() {
        self.labelNoteTitle = CustomLabel()
        self.labelNoteTitle.text = "Заметка"
        self.labelNoteTitle.textColor = .secondaryLabel
        self.addSubview(self.labelNoteTitle)
        
        NSLayoutConstraint.activate([
            self.labelNoteTitle.leftAnchor.constraint(
                equalTo: self.leftAnchor,
                constant: 20),
            self.labelNoteTitle.rightAnchor.constraint(
                equalTo: self.rightAnchor,
                constant: -20),
            self.labelNoteTitle.topAnchor.constraint(equalTo: self.priceStockTextField.bottomAnchor, constant: 8)
        ])
        
        self.noteTextView.translatesAutoresizingMaskIntoConstraints = false
        self.noteTextView.backgroundColor = .secondarySystemGroupedBackground
        ...
        NSLayoutConstraint.activate([
            noteTextView.leftAnchor.constraint(
                equalTo: self.leftAnchor,
                constant: 20),
            noteTextView.rightAnchor.constraint(
                equalTo: self.rightAnchor,
                constant: -20),
            noteTextView.heightAnchor.constraint(
                equalToConstant: 60),
            noteTextView.topAnchor.constraint(
                equalTo: labelNoteTitle.bottomAnchor ,constant: 8)
        ])
    }
    ...
    private func setupWriteOffMoneyLabel() {
        ...
        self.writeOffMoneyLabel.text = "Списать деньги"
        ...
        NSLayoutConstraint.activate([
            ...
            self.writeOffMoneyLabel.topAnchor.constraint(
                equalTo: noteTextView.bottomAnchor ,constant: 8)
        ])
...
extension BuyStockView: UITextFieldDelegate, UITextViewDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField == self.tickerStockTextField
            || textField == self.amountStocksTextField {
            keyBoardObserver?.stopKeyboardObsrever()
        } else {
            keyBoardObserver?.startKeyboardObsrever()
        }
    }
